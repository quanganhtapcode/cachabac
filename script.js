...
class BillSplitter {
  ...
  calculateResults() {
        ...
        item.eaters.forEach(eaterIndex => {
          if (results[eaterIndex]) {
            ...
            results[eaterIndex].details.push({
              type: 'eat',
              itemName: item.name || 'Món không tên',
              amount: costPerEater
            });
          }
        });
    ...
    this.displayResults(results, totalAmount);
  }
  ...
  createResultElement(result) {
    ...
    const detailsText = result.details.map(detail => {
      const type = detail.type === 'eat' ? 'Ăn' : 'Trả';
    ...
    if (isOwed) {
      ...
      // Set up QR button - show for people who OWE money (positive amounts)
    if (isOwed && this.isPaymentInfoComplete()) {
      qrButton.style.display = 'block';
      qrButton.addEventListener('click', () => {
        this.generateQRCode(Math.abs(amount), `Thanh toán tiền ăn - ${result.name}`);
      });
    }
